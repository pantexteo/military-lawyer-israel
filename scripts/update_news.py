...
NEWS_COUNT = 2  # статей за запуск (качество > количество; сниж. с 3 16.05.2026)
...
ПРАВИЛА:
1. Пиши по-русски, простым языком, как для друга. Без канцелярита и сухих новостных штампов («сообщается», «как стало известно», «по данным источников»).
2. Статья 700-900 слов. 3 секции с разными подзаголовками.
3. Объясни что эта новость значит ЛИЧНО для нашей аудитории (репатриант 25-45 лет, женщина или мужчина, числится дезертиром или боится лететь в Израиль). Не пересказывай новость — анализируй её.

4. **УНИКАЛИЗАЦИЯ — ОБЯЗАТЕЛЬНО.** Статья не должна быть «ещё одной перепечаткой».
   Включи минимум 3 из этих 5 элементов (выбери самые подходящие к теме):
   a) **Прямой комментарий Юлии от первого лица** — короткий абзац или цитата в кавычках («Я видела похожую ситуацию у клиента в марте — ...» / «Когда меня саму задержали в Бен-Гурионе, я не понимала, что...»). Привязка к её личному опыту: 4 года жизни в Израиле, медкомиссия, задержание, военная тюрьма.
   b) **Конкретные ссылки на закон Израиля** — статья 92, 94 или 95 Закона об оборонной службе 1986 года; их разница и применение.
   c) **Конкретные цифры в шекелях** — штрафы (от 2 000 до 15 000 шек.), сроки (30-60 суток военной тюрьмы), даты решений судов.
   d) **Анонимизированный мини-кейс из практики** (2-3 предложения) — «В апреле 2026 к нам обратился Сергей, 32 года, прилетел из Москвы — на паспортном контроле его задержали, через 6 часов перевели в Тель ха-Шомер. Дело удалось перевести с ст.94 на ст.92, итог — 30 суток.»
   e) **Сравнение с тем что было год-два назад** — «До октября 2023 военная прокуратура чаще давала amnesty repatriantам. С началом войны Меценах Харбот Барзель практика ужесточилась».

5. **Свой угол подачи.** Не пересказывай новость подряд — выбери ОДИН аспект, важный нашей аудитории, и развей его глубже. Лучше один глубокий аспект, чем поверхностный пересказ всей новости.

6. **Заголовок-крючок.** SEO-заголовок должен ловить страх или интригу пользователя. Не «ЦАХАЛ принял новый закон», а «Новый закон ЦАХАЛ: что это значит для репатриантов из СНГ». Не «80 тысяч уклонистов в Израиле», а «80 тысяч уклонистов: попадаете ли вы под новую волну проверок».

7. Упомяни Юлию и проект 1-2 раза органично — не больше («Если вас это касается, бесплатная проверка статуса — мы делаем её за 1-3 дня»).

8. FAQ — 5 вопросов, которые реально задал бы испуганный человек. Никаких общих «что такое дезертир» — только специфика темы этой статьи.

9. Slug — транслитерация (например prizyv-reservistov-2026), без кириллицы, через дефис, ≤60 символов.

10. Если новость нерелевантна аудитории (политика без юридического аспекта, общеизраильские новости без связи с призывом/дезертирством/аэропортом) — верни: {{"skip": true}}

11. Уже опубликованные темы (НЕ дублировать!): {existing_topics}
    Если новость о той же теме что уже была — верни {{"skip": true}}. Сходство тем — не сходство заголовков. Если предыдущая статья была «закон о призыве харедим», а новая — «БАГАЦ о харедим» — это разные темы, можно писать."""
...
def generate_article(client: OpenAI, item: dict, existing_topics: str = "нет") -> Optional[dict]:
    ...
    try:
        ...
        # === ПРОВЕРКА УНИКАЛИЗАЦИИ ===
        # Собираем весь текст статьи (lead + sections + faq) для анализа
        all_text_parts = [data.get("lead", ""), data.get("card_summary", "")]
        for s in data.get("sections", []):
            all_text_parts.append(s.get("text", ""))
        for f in data.get("faq", []):
            all_text_parts.append(f.get("answer", ""))
        full_text = " ".join(all_text_parts).lower()

        # Считаем сколько unique-сигналов в статье (требуем минимум 2 из 4):
        # — шекели (конкретные цифры)
        # — ссылка на статью закона (ст. 92/94/95 или статья 92...)
        # — личный голос Юлии (юлия, мне, когда меня, я сама)
        # — мини-кейс из практики (имя+возраст+ситуация)
        signals = {
            "money": bool(re.search(r"\bшек(\.|елей|еля)?\b|\d{3,4}\s*₪", full_text)),
            "law": bool(re.search(r"\b(ст\.|статья|статьи)\s*9[245]\b|закон[а-я]*\s+об\s+оборонной\s+служб", full_text)),
            "yulia": bool(re.search(r"юли[яюей]|когда\s+меня|я\s+(сама|видела|знаю)\s|мне\s+самой", full_text)),
            "case": bool(re.search(r"клиент[а-я]*\s+(в|из)|обратил(ся|ась)\s+к\s+нам|кейс\s+\d|в\s+(марте|апреле|мае|июне|июле|январе|феврале)\s+202[56]", full_text)),
        }
        signal_count = sum(signals.values())
        if signal_count < 2:
            missing = [k for k, v in signals.items() if not v]
            print(f"  SKIP (uniqueness: only {signal_count}/4 signals, missing: {missing})")
            return None

        # === ПРОВЕРКА КОНТЕНТНОЙ ПОХОЖЕСТИ С СУЩЕСТВУЮЩИМИ СТАТЬЯМИ ===
        # Берём lead+первая секция новой статьи, сравниваем с lead существующих
        new_signature_text = (data.get("lead", "") + " " + (data.get("sections", [{}])[0].get("text", "") if data.get("sections") else "")).lower()
        new_words = set(re.findall(r"\b[а-яё]{4,}\b", new_signature_text))
        if new_words:
            max_sim = 0.0
            max_match = ""
            for slug in get_existing_slugs():
                path = os.path.join(NEWS_DIR, f"{slug}.html")
                try:
                    with open(path, encoding="utf-8") as f:
                    ...
                    continue
                existing_text = re.sub(r"<[^>]+>", " ", existing_html).lower()
                existing_words = set(re.findall(r"\b[а-яё]{4,}\b", existing_text))
                ...
                if sim > max_sim:
                    ...
                    max_match = slug
            if max_sim >= 0.55:
                print(f"  SKIP (content similarity {max_sim:.2f} with '{max_match}')")
                return None
